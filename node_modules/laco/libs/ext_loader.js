...
let loadInstalledExts = (laco, extJsonFile, loadExtCallback) => {
    jsonfile.readFile(extJsonFile, function(err, appExtObj) {
        ...
        loadAvailablePackagesJson((err, availableExts) => {
            ...
            async.map(installedExtFiles, (extFile, callback) => {
                ...
                try {
                    listedExts[extFile] = require(extFile);
                    if(_.isFunction(listedExts[extFile])) {
                        loadedExts[extBaseName] = new listedExts[extFile](laco);
                        callback(null, extBaseName);
                    }
                    else {
                        logger.error(`[ExtLoader.loadInstalledExts] ${extBaseName} ext file must return a function or constructor`);
                        callback(null, null);
                    }
                }
                catch(err) {
