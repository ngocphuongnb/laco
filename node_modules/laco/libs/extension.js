...
class Extension {
    ...
    registerViews(views) {
        ...
        async.map(views, (view, callback) => {
            ...
            logger.info(`[Extension.${this.extName}] Registering view for extension "${this.extName}" ${view}`);
            try {
                this.viewsObj[view] = require(viewFile);
                callback(null);
            }
            catch(e) {
                //logger.error(`[Extension.registerViews]--> ${this.extName}: View '${view}' not found, with message: ${e.message}`);
                logger.error(`[Extension.registerViews]--> ${this.extName}: Error registering view: '${view}'`);
                callback(null, null);
            }
        }, (err, _views) => {
            if(err) {
    ...
    view(viewName, data, req, res) {
        ...
        data._common.message = req.flash();
