...
const httpMethods = ['get', 'post', 'put', 'delete', 'head', 'all'];
    ...
    _.each(routeList, routeObj => {
        ...
        if(!!routesWithSamePath.length) {
            ...
            _.each(routesWithSamePath, existedRoute => {
                if(existedRoute.get && routeObj.get) duplicatedMethods.push('GET');
                if(existedRoute.post && routeObj.post) duplicatedMethods.push('POST');
                if(existedRoute.head && routeObj.head) duplicatedMethods.push('HEAD');
                if(existedRoute.delete && routeObj.delete) duplicatedMethods.push('DELETE');
                if(existedRoute.put && routeObj.put) duplicatedMethods.push('PUT');
                if(existedRoute.all && routeObj.all) duplicatedMethods.push('ALL');
            });
