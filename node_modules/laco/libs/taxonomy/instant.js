...
let instant = class TaxonomyInstant {
    ...
    save(data) {
        this.data = data;
        if(!this.data.parent) delete this.data.parent;
        let savedObj = this.id ? provider.update(data, {intId: this.id}) : provider.create(data);

        return new Promise((resolve, reject) => {
            savedObj.then(tax => {
                this.editUrl = `/${process.env.APP_ADMIN_DIR}/taxonomy/add/${tax.intId}`;
                this.data = tax;
                return resolve(tax);
            }).catch(e => {
