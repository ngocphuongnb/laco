...
let start = (laco) => {
    lasso.configure({
        plugins: [
            //'lasso-less',
            require.resolve('lasso-sass'),
            require.resolve('lasso-marko')
        ],
        outputDir: path.join(process.env.APP_ROOT, 'public/static'),
        urlPrefix: '/static',
        bundlingEnabled: !!nconf.get('ASSETS_BUNDLING'),
        minify: !!nconf.get('ASSETS_MINIFY'),
        fingerprintsEnabled: !!nconf.get('ASSETS_FINGERPRINT')
    });
...
let boot = () => {
    ...
    laco.app.use(session({
        ...
        cookie: { secure: false },
        name: nconf.get('APP_COOKIE_NAME')
    }));
    ...
    laco.app.use(expressPoweredBy('Node.js/Laco 1.0.0'));
    laco.app.use(connectFlash());
    laco.app.use(markoExpress());

    //Enable csrf protection
    laco.app.use((req, res, next) => laco.csrfProtection(req, res, next));

    //Enable paging function
    laco.app.use((req, res, next) => {
        let currentUrl = req.url;
        let reg = new RegExp(`\\/${nconf.get('ROUTER_PAGING_WORD')}\\-(\\d+)`, 'g');
        let matches = currentUrl.match(reg);
        req.paging = {
            queries: req.query
        };
        if(matches) {
            let matchPageStr = matches[0];
            req.paging.current = 1*matchPageStr.split('-')[1];
            req.paging.baseUrl = url.parse(currentUrl.replace(reg, '')).pathname;
            req.paging.originalUrl = req.url;
            if(req.paging.baseUrl == '') req.paging.baseUrl = '/';
            req.url = req.paging.baseUrl;
            if(req.paging.current == 1) return res.redirect(req.paging.baseUrl);
            next();
        }
        else {
            req.paging.current = 1;
            req.paging.baseUrl = url.parse(req.url).pathname;
            req.paging.originalUrl = req.url;
            next();
        }
    });

    //Check for csrf on required http methods
    ...
    if(!nconf.get('APP_USE_PROXY') || nconf.get('APP_USE_PROXY') == 'none') laco.app.use('/', laco.express.static(path.join(process.env.APP_ROOT, 'public')))
