...
class UIBase extends EventEmitter {
    ...
    discovery(callback) {
        ...
        async.each([this.options.viewDir, this.options.layoutDir], (scanDir, cb) => {
            ...
            if (err) {
                logger.error(`[lib.ui.baseTask] [${this.ui.name}] Discovery error: `, { task, caller });
            }
            callback(err);
        });
    }
    ...
    addTasks(tasks) {
        ...
        for (var i = 0; i < tasks.length; i++) {
            ...
            if (!task.name) {
                logger.error(`[lib.ui.baseTask] [${this.ui.name}] Task name cannot be empty! Registered from: `, { task, caller });
            } else if (!!taskWithSameName.length) {
                logger.error(`[lib.ui.baseTask] [${this.ui.name}] Task name existed: ${task.name} Registered from: `, { task, caller });
            } else {
                logger.info(`[lib.ui.baseTask] [${this.ui.name}] Registering UI task: ${task.name}`);
    ...
    addLayouts(layouts) {
        ...
        for (var i in layouts) {
            ...
            if (helper.isFileExisted(layoutPath)) {
                logger.info(`[lib.ui.addLayou] [${this.ui.name}] Registering layout: ${layoutName}`);
                layoutName = layoutName.replace(/\//gim, '.');
                this.layouts[layoutName] = require(layoutPath);
            } else {
                let caller = helper.caller();
                logger.error(`[lib.ui.addLayou] [${this.ui.name}] Layout not found: `, { layoutPath, caller });
            }
        }
    }
    ...
    addViews(views) {
        ...
        for (var i in views) {
            ...
            if (helper.isFileExisted(viewPath)) {
                logger.info(`[lib.ui.addViews] [${this.ui.name}] Registering view: ${viewName}`);
                viewName = viewName.replace(/\//gim, '.');
                this.views[viewName] = require(viewPath);
            } else {
                let caller = helper.caller();
                logger.error(`[lib.ui.addViews] [${this.ui.name}] View not found: `, { viewPath, caller });
            }
        }
    }
}
