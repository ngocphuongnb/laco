...
class UIBase extends EventEmitter {
    ...
    discovery(callback) {
        ...
        async.each([this.options.viewDir, this.options.layoutDir], (scanDir, cb) => {
            ...
            if (err) {
                logger.error(`[lib.ui.baseTask] [${this.subapp.name}] [${this.ui.name}] Discovery error: `, { task, caller });
            }
            callback(err);
        });
    }
    ...
    addTasks(tasks) {
        ...
        for (var i = 0; i < tasks.length; i++) {
            ...
            if (!task.name) {
                logger.error(`[lib.ui.baseTask] [${this.subapp.name}] [${this.ui.name}] Task name cannot be empty! Registered from: `, { task, caller });
            } else if (!!taskWithSameName.length) {
                logger.error(`[lib.ui.baseTask] [${this.subapp.name}] [${this.ui.name}] Task name existed: ${task.name} Registered from: `, { task, caller });
            } else {
                logger.info(`[lib.ui.baseTask] [${this.subapp.name}] [${this.ui.name}] Registering UI task: ${task.name}`);
