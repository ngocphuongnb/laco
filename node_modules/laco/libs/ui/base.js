...
class UIBase extends EventEmitter {
    constructor(manager, ui, options = {}) {
        ...
        this.laco = this.manager.laco;
        // this = ui;
        let caller = helper.caller();
        this.extDir = caller.dir;

        let extDirParts = caller.dir.split('/');

        while (extDirParts.length >= 0) {
            let currentDir = extDirParts.pop();

            if (currentDir.startsWith('laco-ext-')) {
                extDirParts.push(currentDir);
                this.extDir = extDirParts.join('/');
                break;
            }
        }
    ...
    discovery(callback) {
        ...
        async.each([this.options.viewDir, this.options.layoutDir], (scanDir, cb) => {
            ...
            if (err) {
                logger.error(`[lib.ui.baseTask] [${this.name}] Discovery error: `, { task, caller });
            }
            callback(err);
        });
    }
    ...
    addTasks(tasks) {
        ...
        for (var i = 0; i < tasks.length; i++) {
            ...
            if (!task.name) {
                logger.error(`[lib.ui.baseTask] [${this.name}] Task name cannot be empty! Registered from: `, { task, caller });
            } else if (!!taskWithSameName.length) {
                logger.error(`[lib.ui.baseTask] [${this.name}] Task name existed: ${task.name} Registered from: `, { task, caller });
            } else {
                logger.info(`[lib.ui.baseTask] [${this.name}] Registering UI task: ${task.name}`);
    ...
    addLayouts(layouts) {
        ...
        for (var i in layouts) {
            ...
            if (helper.isFileExisted(layoutPath)) {
                logger.info(`[lib.ui.addLayou] [${this.name}] Registering layout: ${layoutName}`);
                layoutName = layoutName.replace(/\//gim, '.');
                this.layouts[layoutName] = require(layoutPath);
            } else {
                let caller = helper.caller();
                logger.error(`[lib.ui.addLayou] [${this.name}] Layout not found: `, { layoutPath, caller });
            }
        }
    }
    ...
    addViews(views) {
        ...
        for (var i in views) {
            ...
            if (helper.isFileExisted(viewPath)) {
                logger.info(`[lib.ui.addViews] [${this.name}] Registering view: ${viewName}`);
                viewName = viewName.replace(/\//gim, '.');
                this.views[viewName] = require(viewPath);
            } else {
                let caller = helper.caller();
                logger.error(`[lib.ui.addViews] [${this.name}] View not found: `, { viewPath, caller });
            }
        }
    }
}
