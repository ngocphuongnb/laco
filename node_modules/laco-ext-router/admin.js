...
exports.middleware = () => {
    router.use((req, res, next) => {
        if(!req.session.user || !req.session.user.can('access_admin')) return res.redirect(`/login?back=${encodeURIComponent(req.originalUrl)}`);
        next();
    });
    router.get('/:module?/:action?/:extra([a-zA-Z0-9\-\_\/]+)?', function(req, res) {
        admin.adminObjects.dashboard.main(req, res);
